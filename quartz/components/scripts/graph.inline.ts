...
async function renderGraph(container: string, fullSlug: FullSlug) {
	...
	const isKnowledgeGraphMode =
		toggleElement?.checked ?? showKnowledgeGraph ?? false;
		...
		if (currentPageData) {
			...
			if (currentPageData.frontmatter?.relationships) {
				for (const rel of currentPageData.frontmatter.relationships) {
					const targetSlug = simplifySlug(
						rel.object?.replace(/\.md$/, "") as FullSlug,
					);
					links.push({
						source: slug,
						target: targetSlug,
				...
				for (const triple of currentPageData.frontmatter.semantic_triples) {
					...
					if (triple.subject === "self") {
						...
						if (triple.object?.endsWith?.(".md")) {
							targetSlug = simplifySlug(
								triple.object.replace(/\.md$/, "") as FullSlug,
							);
						} else if (triple.object) {
							// Convert entity names to valid slugs (handle spaces)
							targetSlug = simplifySlug(
								triple.object.toLowerCase().replace(/\s+/g, "-") as FullSlug,
							);
						}
						...
						if (triple.subject?.endsWith?.(".md")) {
							sourceSlug = simplifySlug(
								triple.subject.replace(/\.md$/, "") as FullSlug,
							);
						} else if (triple.subject) {
							// Convert entity names to valid slugs (handle spaces)
							sourceSlug = simplifySlug(
								triple.subject.toLowerCase().replace(/\s+/g, "-") as FullSlug,
							);
						}
					} else {
						// Triples between other entities mentioned in current page
						if (
							triple.subject?.endsWith?.(".md") &&
							triple.object?.endsWith?.(".md")
						) {
							sourceSlug = simplifySlug(
								triple.subject.replace(/\.md$/, "") as FullSlug,
							);
	...
	function animate(time: number) {
		...
		for (const l of linkRenderData) {
			...
			if (hasValidCoords) {
				...
				if (
					l.simulationData.type === "semantic_triple" ||
					l.simulationData.type === "relationship"
				) {
